...
import com.javatrain.bbs.dao.ReplyDao;
...
public class ReplyDaoImpl extends BaseDao implements ReplyDao {
	...
	public List findListReply(int page, int topicId) {
        ...
        if( page > 1 ) {
            ...
            sql = "select * from TBL_REPLY where topicId=" + topicId + " and replyId not in(select replyId from TBL_REPLY where topicId=" + topicId + "order by publishTime limit "+ rowBegin +")order by publishTime limit 10";
        }else
        	sql ="select * from TBL_REPLY where topicId=" + topicId + " order by publishTime limit 10";
        try {
            conn = this.getConn();                     // 得到数据库连接
           // String sql = "select * from TBL_REPLY where topicId=" + topicId + " and replyId not in(select replyId from TBL_REPLY where topicId=" + topicId + "order by publishTime limit "+ rowBegin +")order by publishTime limit 10";
